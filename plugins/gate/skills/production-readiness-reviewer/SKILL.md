...
**The supported workflow creates N/A records only through the canonical `record_evidence.py --not-applicable` operation.** It derives the commit itself, validates the matrix cell, rejects mandatory categories, and generates the timestamps. The schema requires the `recorder` format label on the N/A branch, so omitting or changing it fails validation; copying that label into otherwise conforming JSON is still possible and is not proof of origin. Each record must carry a recognized `profile` matching the project's recorded profile, a substantive `reason` (>= 20 characters and >= 4 words — one character is a rejection), a non-empty `reviewer`, and a **structured `applicability` object** naming the matrix cell, where the profile is recorded, and what was actually inspected:
