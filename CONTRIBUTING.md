...
For v1.0.24, publish in two reviewed stages from PowerShell. Supply the remote
...
```powershell
& .\release\publish-approved-tag-v1.0.24.ps1 `
  -RemoteUrl $remote `
  -ApprovedCommitOid "<approved 40-hex OID>"
```

That helper requires `release/v1.0.24` still to equal the approved OID and
